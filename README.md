...
The platform transitions from a monolithic architecture to a completely decoupled, event-driven, and highly resilient microservices pipeline connected via **Redis Streams** and backed by **PostgreSQL** and **Apache Kafka (Redpanda)**.
...
flowchart TD
    ...
    Gateway -->|Save source & meta| PG[(PostgreSQL)]
    Gateway -->|Publish compilation job| R1[Redis Stream: compilation_queue]
    
    subgraph Compiler Service
        R1 --> Compiler[Compiler Worker]
        Compiler -->|Execute compile container| DockerCompile[Docker Sandbox: g++]
        DockerCompile -->|Write binary /app| SharedVolume[(Shared Directory)]
        Compiler -->|Publish pretest job| R2[Redis Stream: pretest_queue]
    end
    
    subgraph Pretest Service
        R2 --> Pretest[Pretest Worker]
        Pretest -->|Query binary| SharedVolume
        Pretest -->|Spin up runtime container| DockerRuntime[gVisor Sandbox: App]
    ...
    staticLeaderboard -->|Poll JSON| CDN[Contestant Browser Standing UI]
```

### Microservices Decomposition

* **Submission Gateway** (`services/gateway/`): Stateless Fiber web server handling submission uploads, performing Redis-based rate limiting (1 submission per user/minute), persisting metadata/source code in Postgres, and dispatching jobs to the `compilation_queue` Redis Stream.
* **Compiler Service** (`services/compiler/`): An event loop polling the `compilation_queue` stream. Executes a standard compilation task inside an ephemeral Docker container. Runs using host-owner UID matching to resolve file permission conflicts, generating a static `app` binary written to a shared directory.
* **Pretest Service** (`services/pretest/`): An event loop polling the `pretest_queue` stream. Spins up the contestant binary in a sandboxed gVisor container under strict resource restrictions and seccomp filters. Evaluates correctness and basic performance using an in-process 5-bot fleet (100 orders each, fixed seed) over a WebSocket connection. Logs results to PostgreSQL.
* **Leaderboard Generator** (`services/leaderboard/`): Periodically queries PostgreSQL to calculate compound scores and ranks, materializing rankings to a global static file (`frontend/leaderboard.json`) every 3 seconds to support 100K pollers via CDN caching at zero database load.
* **Developer Diagnostics Dashboard**: An interactive glassmorphism UI served directly at `http://localhost:3000/dashboard` showing HTTP request rates, active compiler/pretest queue sizes, database/broker metrics, and detailed submission traces with code inspection drawer.

---

## 2. Secure Sandboxing Model

Contestant C++ matching engines are treated as untrusted user code. To prevent host compromise, denial of service, or platform spoofing, a multi-layer isolation envelope is enforced during compilation and execution.
...
```
       ...
       |  |  |  [network: airgapped sandbox]   |  |  |
       |  |  |  [seccomp: strict syscalls]     |  |  |
       |  |  |                                 |  |  |
...
1. **Kernel Sandboxing (gVisor)**: Run containers under the `runsc` OCI runtime, which intercepts all system calls and processes them in a user-space kernel written in Go, preventing direct execution on the host kernel.
...
3. **Capabilities & System Calls (Seccomp)**: Enforces a custom `seccomp.json` filter dropping all Linux capabilities (`CapDrop: ["ALL"]`) and explicitly blocking system calls related to process creation (`fork`, `vfork`, `execve`, `execveat`) and file system mutations.
4. **Dynamic Port Mapping (`HostPort: "0"`)**: Containers execute on the private isolated `sandbox-net` bridge network. To allow the host pretest worker to talk to the engine without breaking seccomp, the container exposes port `8080` (where the matching engine listens) and binds it to localhost with port `0`. Docker allocates a random high port on the host, preventing collisions and allowing concurrent runner tasks.
...
* **Problem**: During early pretest runs, containers exited immediately with status code `159` and produced no logs. This was traced to `SIGSYS` (Bad System Call), triggered when the OCI runtime (`runc`) or glibc attempted namespace adjustments in host-network mode under our strict seccomp profile, which blocks namespace and process execution syscalls.
* **Resolution**: Replaced host networking with the bridge `sandbox-net` network and implemented **Dynamic Port Mapping**. By binding `8080/tcp` to `127.0.0.1:0`, Docker automatically allocates a free host port, allowing secure loopback websocket connectivity without kernel capability escalations.
...
├── migrations/                 # Postgres database schema migrations
├── scripts/                    # Automation scripts for testing and orchestration
├── services/                   # Go-based microservices
│   ├── common/                 # Shared helpers for Docker client connections, Redis pools, and Seccomp definitions
...
│   └── leaderboard/            # standings JSON generator
...
Start Postgres, Redis, and Redpanda in the background:
```bash
docker compose up -d
```
...
### 4. Running the End-to-End (E2E) Test Suite
...
### 5. Running High-Load Post-Contest System Tests
...
### 6. Accessing the Developer Diagnostics Dashboard
